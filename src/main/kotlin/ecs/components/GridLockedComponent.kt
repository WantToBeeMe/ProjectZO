...
class GridLockedComponent {
    ...
    fun getGLCLeftTopIndex(gLMousePos: Vector2f, gs : GridSettings) : Vector2i {
        ...
        val mostLeft =if(gs.lockYaxis) -(scaledBlockSize * gs.gridWidth) / 2 else -1f*zoom + gs.borderWidth * zoom
        val mostTop = if(gs.lockYaxis) 1f*zoom - gs.borderWidth * zoom else (scaledBlockSize * gs.gridHeight) / 2
